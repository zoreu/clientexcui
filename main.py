...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_path.path == '/' or parsed_path.path == '/index.html':
            ...
            self.end_headers()
            self.wfile.write(b'<h1>404 - Página não encontrada</h1>')
...
def start_server():
    ...
    print(f"   • Local (mesmo dispositivo): http://localhost:{PORT}")
    print(f"   • Rede local (outros dispositivos): http://{local_ip}:{PORT}")
    print(f"\n💡 DICAS:")
